...
- Power handling peaks around 25–30 A continuous (≈1.5 kW on 13S). Plan controller and wiring upgrades if firmware demands higher draw or hill-climb duty.[^3][^4]
- Newer hardware revisions add charger and regen over-voltage protection, yet legacy boards rely entirely on pack BMS behavior. Treat anything above 42 V as advanced use that still requires conservative charge targets (~95 % state of charge).[^5]
...
| 13S externals | ⚠️ Experimental | Rita reports up to 15S, but current remains capped around 25 A; mixing 15S externals with 10S internals offers little benefit and risks cutoffs.[^4]
...
| Counterfeit auxiliary packs | Capacity sag, overheated wiring, or exaggerated Ah ratings. | Source from vetted builders (Samsung 35E/22PM cells), inspect weld quality, and secure the bag with clamps or cages.[^31][^32]
...
[^31]: `knowledge/notes/all_part01_review.md`, lines 41-48, 60-68.
...
[^37]: `knowledge/notes/all_part01_review.md`, lines 30-33, 60-68.
[^38]: `knowledge/notes/all_part01_review.md`, lines 31-35, 87-91.
