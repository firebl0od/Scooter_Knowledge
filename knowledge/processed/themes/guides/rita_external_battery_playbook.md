...
3. **Route and secure leads**: Keep XT30 connectors strain-relieved, add anti-spark switches between Rita and the controller if desired, and avoid repeated hot-swaps.[^lead-care]
...
6. **Perform voltage and regen checks**: Verify recuperation-off voltage (≈4.15 V for 12 S) and confirm throttle response using conservative intensity-of-current-change values (~300–350 mA) before raising power constants.[^regen-check]
...
[^charge-flow]: Source: `knowledge/notes/all_part01_review.md`, lines 54, 172.
...
[^range-planning]: Source: `knowledge/notes/all_part01_review.md`, lines 229, 199.
