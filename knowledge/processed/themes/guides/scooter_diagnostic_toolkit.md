...
6. **Hand-tap new mounts.** When fabricating caliper or adapter holes, clamp the frame and turn taps by hand—power drilling snaps them instantly. If a tap breaks, enlist a local shop or braze an adapter rather than forcing it with hand tools.【F:knowledge/notes/input_part005_review.md†L304-L305】
7. **Secure accessory power taps.** Dashboards only expose 5 V logic—drive headlights and horns from a dedicated DC/DC converter, tie the output wires to the board with RTV, and route the harness away from moving stems so the leads do not fatigue.【F:knowledge/notes/denis_all_part02_review.md†L28-L32】【F:knowledge/notes/denis_all_part02_review.md†L31-L32】
...
- **Work a MOSFET detection checklist:** If motor detection instantly trips or blows FETs, pause and meter gate drivers, inspect phase filters for solder debris, and rerun detection with conservative amps/ERPM before reapplying pack power—the community recovered multiple 30-pole hubs by methodically clearing each step.【F:knowledge/notes/input_part005_review.md†L611-L611】
