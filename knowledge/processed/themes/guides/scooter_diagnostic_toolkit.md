...
- **Bench-test halls before cracking motors:** Inexpensive AliExpress testers confirm hall outputs without dismantling sealed hubs, catching sensor faults before you chase observer settings.【F:knowledge/notes/input_part012_review.md†L433-L433】
- **Land thermistors on temp+/GND pads:** VESC temp probes belong on the controller’s temperature and ground pins, not the 5 V rail, to avoid skewed readings or shorts during diagnostics.【F:knowledge/notes/input_part012_review.md†L433-L433】
- **Populate regen limits in VESC Tool.** Motor Current Max Brake/Max Regen values must be set before regen works; otherwise braking looks dead despite healthy hardware.【F:knowledge/notes/input_part012_review.md†L433-L433】
- **Validate thermal calibration when cutbacks arrive early.** If a controller starts throttling ~10 °C below the configured limit (e.g., 91 °C throttle and 95 °C shutdown despite a 105 °C setting), double-check the temp-sensor wiring and firmware clamps before assuming hardware failure.【F:knowledge/notes/input_part012_review.md†L469-L470】
- **Log traction-control slip mid-corner.** Riders still debate whether VESC traction control stabilises leans or just blocks burnouts—log per-wheel speed and slip ratio during cornering before promising stability gains to customers.【F:knowledge/notes/input_part012_review.md†L468-L468】
