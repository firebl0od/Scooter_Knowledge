...
4. **Clean the white battery plug after water rides.** Oxidation hides under the connector and causes power resets or false-low readings until you flush with contact cleaner and dry thoroughly.【F:knowledge/notes/denis_all_part02_review.md†L309-L309】
5. **Diagnose dark dashboards at the controller.** If the dash button still bridges ground but the ESC status LED stays dark, trace the controller power-supply stages, regulators, and coil drops before assuming the dashboard died—the switch is only a momentary signal line.【F:knowledge/notes/denis_all_part02_review.md†L94-L95】
6. **Treat locks as consumables:** Light cable locks such as compact Master Lock loops work for coffee stops only if they stay lubricated with graphite—dry mechanisms jam quickly after rain or grit exposure.【F:knowledge/notes/all_part01_review.md†L191-L191】
7. **Identify controller silicon before flashing.** New Xiaomi ESCs use GD32 MCUs; flashing STM firmware bricks them, so zoom in on chip markings and grab the GD image set before running ST-Link recoveries.【F:knowledge/notes/denis_all_part02_review.md†L95680-L95780】
8. **Secure accessory power taps.** Dashboards only expose 5 V logic—drive headlights and horns from a dedicated DC/DC converter, tie the output wires to the board with RTV, and route the harness away from moving stems so the leads do not fatigue.【F:knowledge/notes/denis_all_part02_review.md†L28-L32】【F:knowledge/notes/denis_all_part02_review.md†L31-L32】
9. **Trace weak 5 V accessory ports back to the buck converter.** A Max G30 plug that still shows 5 V but no current usually means a failed converter stage—follow the trace with a multimeter until you reach the DC/DC module and verify components between it and the port before ordering a replacement board.【F:data/E-scooter upgrade workshop by denis yurev/text_slices/all.part02.txt†L110038-L110041】
...
- **Treat error 28 after water as a controller failure.** Shorted high-side bridges leave pink stickers and white corrosion; swap the controller instead of chasing intermittent faults once the scooter has flooded.【F:knowledge/notes/denis_all_part02_review.md†L308-L308】
...
- **Disable traction control on slick surfaces during AWD tests.** Roscheeee’s VESC dual-motor G30 snapped Tronic controllers when TCS dumped an estimated 250 A back through the system—log wheel speed manually before trusting traction control.【F:knowledge/notes/denis_all_part02_review.md†L489-L490】
...
- **Red-blinking Happy BMS after a hard brake:** Isolate the controller, safely bleed the main capacitors on a non-conductive surface, and meter the power stage before reconnecting to avoid compounding a short.【F:knowledge/notes/denis_all_part02_review.md†L536-L537】
