...
- **Generic JP TF/TS100 harness.** The unlabeled “generic JP” screen shares the TF/TS100 button colour order—just map each lead to the controller loom—and upcoming revisions add native CAN, SmartDisplay-style telemetry, and ESP32-C3 Express headers for Wi‑Fi/BLE features out of the box.[^jp-generic]
- **Voyage “Megan” CAN dash.** Voyage Systems sells the Megan panel directly; it boots on CAN first, pairing cleanly with lighting-heavy builds that want to avoid UART bandwidth limits.[^voyage-megan]
- **Voyage dash mounting kit.** Pandalgns published a three-piece STL set that clamps the CAN display to 30–32 mm handlebars and adds printable retention straps so polished installs stay secure without metal brackets.【F:knowledge/notes/input_part010_review.md†L102-L104】
- **Spintend illuminated pod caution.** The factory-lit handlebar pod backfeeds voltage into the ADC board—treat it as cosmetic only unless you rewire each switch to isolated on/off channels.[^spintend-pod]
