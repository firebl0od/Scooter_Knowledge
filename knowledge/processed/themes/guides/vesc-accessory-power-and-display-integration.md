...
7. **Split hall leads for OEM dashboards.** Keep the five standard hall wires feeding both the VESC and stock controller so the factory dash still counts speed; IO Hawk gear hubs even add a sixth tach hall dedicated to the display signal.【F:knowledge/notes/input_part004_review.md†L166-L167】
8. **Let retired controllers host accessories.** A VESC with a healthy DC/DC stage can run BLE dongles, dashboards, or CAN bridges even after the power stage is removed—use them as telemetry hubs during repairs or travel delays.【F:knowledge/notes/input_part004_review.md†L331-L332】
9. **Key switches should interrupt the DC/DC enable, not throttle wires.** Breaking signal lines leaves scooters stuck at full duty when the wire reconnects—cut controller enable power or force an e-brake input instead.【F:knowledge/notes/input_part004_review.md†L300-L300】
...
6. **Budget LED strip draw.** A 144-count WS2812B string pulls roughly 2–3 A at 5 V and can hang off a microcontroller directly when you follow Adafruit-style wiring—keep those loads off fragile logic rails without a dedicated buck.【F:data/vesc_help_group/text_slices/input_part004.txt†L15448-L15472】
7. **Size lighting looms for 12 V loads.** Brake + headlight pairs routinely pull 4–6 A; run 16–18 AWG copper with soldered joints and heatshrink instead of undersized silicone or WAGO taps to avoid melted insulation.【F:knowledge/notes/input_part004_review.md†L248-L248】【F:data/vesc_help_group/text_slices/input_part004.txt†L17752-L17764】
8. **Compare onboard vs. external lighting power.** Spintend’s 3 A rail survives tail-lights and indicators, but dual 12 V headlights or LED strips overwhelm it—Flipsky-class controllers expect an external buck to shoulder anything above ~30 W.【F:data/vesc_help_group/text_slices/input_part004.txt†L3970-L3985】
