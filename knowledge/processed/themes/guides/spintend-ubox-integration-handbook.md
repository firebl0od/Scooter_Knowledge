...
6. **Confirm switch polarity and precharge sizing:** The latching button ships with inverted LED logic compared with aftermarket toggles, and 1 kΩ precharge resistors ran hot on high-capacitance decks—rewire polarity and drop to faster resistors or full anti-spark hardware before sealing the loom.【F:knowledge/notes/input_part002_review.md†L12159-L12179】【F:knowledge/notes/input_part002_review.md†L12457-L12463】
...
- **Machine housings accordingly:** Swap thick pads for thin Kapton plus paste on CNC housings, slip thin washers under printed coolers for airflow, and note that Laotie ES19 and Vsett 10 hubs share the bolt pattern for dual-sided heatsinks.【F:knowledge/notes/input_part002_review.md†L356-L357】
- **Potting and heat pipes help but don’t replace airflow:** Potting compound, deck clamps with fresh paste, and heat pipes into thicker aluminium plates cut temps by ≈20 °C, yet 220–250 A bursts still blow traces—plan external mounts or fans for extreme tunes.【F:knowledge/notes/input_part002_review.md†L357-L358】
...
- **Traction-control heat tax:** Enable traction control on both controllers over CAN and budget for higher case temperatures—field logs show peak power dropping from ~18 kW to ~13 kW when the current algorithm overcorrects, so monitor temps before declaring victory on tyre wear.[^traction-heat]
...
- **Respect the 17 S ceiling.** Spintend reiterated that 17 S (regen off) is the practical limit—18 S pushes and 220–250 A surges keep blowing traces even with copper blocks and fans, so daily tunes settle near 200 A per channel until airflow and mounting are overbuilt.【F:knowledge/notes/input_part002_review.md†L335-L338】
...
- **Adapter V2 quick toggles:** Holding the left brake and throttle during power-up drops the adapter into a reduced-power mode with audible feedback; remember that throttle movement still will not register until VESC Tool’s input app is set to ADC, even if a multimeter shows hall voltage swing.[^adapter-v2-toggle]
...
- **Parallel bucks carefully:** If auxiliary lighting needs more than ~3 A at 12 V, paralleling buck converters or adding an external battery is safer than overloading the onboard rail—teams now run twin bucks or standalone packs when horns and high-power lamps share the loom.【F:knowledge/notes/input_part002_review.md†L374-L376】
...
- **CNC throttle rollout:** Spintend’s $59 CNC thumb throttle ships with an optional $17 top-button module, onboard MCU for 3.3 V bidirectional output or 5 V legacy mode, discrete e-brake trigger, cruise toggle input, and removable button pod; early testers love the regen ergonomics but warn that shielded cabling may be required above ~100 A phase to keep 3.3 V ADC lines quiet.[^cnc-throttle]

## Firmware, Logging & Fault Recovery
- **Mainline firmware now includes uBox profiles.** Spintend merged the dual and single hardware into stock VESC firmware 6.0, so you can flash official releases instead of the legacy 5.3 branch—both the micro-USB duals and newer USB‑C boards are covered.【F:knowledge/notes/input_part002_review.md†L616-L617】
...
- **Recover stubborn BLE modules over USB.** If the uBox stops advertising after updates, flash both firmware and bootloader from Spintend’s shared Google Drive via USB, then re-enable UART/BLE toggles in VESC Tool before rebooting—the module stays silent until that sequence is finished.【F:data/vesc_help_group/text_slices/input_part002.txt†L13177-L13277】
...
- **Red USB-C voltage fix:** FW 5.3 on the red USB-C revision misreported pack voltage until riders flashed Spintend’s corrected BIN (`VESC_UBOX_75_100_TYPEC_R2_3.3V_100A_FW5.3.bin`); apply it before chasing hardware faults.[^typec-voltage]
- **Keep the CAN jumper for SWD rescues.** The bundled four-pin CAN cable matches the SWD pinout (reset is unused), so you can bridge into SWD mode, install STM32 drivers, and reflash dead logic without waiting on replacement harnesses.【F:knowledge/notes/input_part002_review.md†L235-L236】
- **Document mixed-controller harnesses.** When powering external Rion or other controllers from the Ubox loom, map precharge, connector gauge, and balance wires carefully so the shared pack cannot backfeed a dormant ESC mid-ride.【F:knowledge/notes/input_part002_review.md†L338-L338】
...
- **Diagnose dead MCUs by SWD voltage.** If the SWD header only shows ~0.6 V on VCC instead of 3.3 V, Spintend support treats the controller as a failed MCU that needs an RMA—do not keep flashing once you see sub-1 V rails.[^swd-low-vcc]
- **Expect ERPM cut-outs near the 140 A absolute ceiling.** 16 S5 P builds touching 115 km/h have tripped ABS overcurrent around 140 A even with higher limits configured, and pack sag toward −15 V on 120 A tunes confirms connectors and cells need upgrades before blaming the BMS.【F:knowledge/notes/input_part002_review.md†L178-L180】【F:knowledge/notes/input_part002_review.md†L180-L181】
...
- **Single 100 V refresh in flight.** Spintend paused the single-Ubox launch to swap a flawed brass heat spreader for copper and is evaluating bare aluminium casings plus broader thermal tweaks—expect longer lead times while the revision bakes.【F:knowledge/notes/input_part002_review.md†L226-L228】
...
[^adapter-v2-toggle]: Adapter V2 adds a reduced-power throttle-scaling mode triggered by holding the left brake and throttle during power-on and still requires VESC Tool’s input app to be set to ADC before the hall output registers.【F:knowledge/notes/input_part002_review.md†L19-L22】
[^cnc-throttle]: Spintend confirmed the $59 CNC throttle (with optional $17 top button) ships with an onboard MCU that handles 3.3 V bidirectional or 5 V-scaled output, discrete regen trigger, cruise input, and removable button modules; beta testers recommend shielded cabling on ≥100 A phase builds to keep the 3.3 V ADC line quiet.【F:knowledge/notes/input_part002_review.md†L196-L200】
[^typec-voltage]: FW 5.3 misread voltage on red USB-C Uboxes until riders flashed the corrected BIN before reconnecting VESC Tool, eliminating the phantom low-voltage alarms.【F:knowledge/notes/input_part002_review.md†L122-L124】
[^traction-heat]: Traction control must be enabled on both controllers, extends front-tyre life, but doubled case temperature and cut peak power from ~18 kW to ~13 kW in recent field logs.【F:knowledge/notes/input_part002_review.md†L50-L52】
[^swd-low-vcc]: Spintend support confirmed that seeing ~0.6 V instead of 3.3 V on the SWD VCC pin indicates a dead MCU that needs an RMA rather than further flashing attempts.【F:knowledge/notes/input_part002_review.md†L42-L44】
